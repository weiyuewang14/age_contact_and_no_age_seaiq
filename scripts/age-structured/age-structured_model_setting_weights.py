...
# %% 将得到的结果读入csv文件中
def write_data(location, state, value, p,p_v,l,l_v,k,k_v,num_agebrackets=18, overwrite=True):
...
# %% 将得到的结果读入csv文件中
def write_data_bracket_age(location, state, value, age, p,p_v,l,l_v,k,k_v,num_agebrackets=18, overwrite=True):
...
# %% 保存每日新增病例数
def write_per_day_new_cases(location, new_case_per_day, p, p_v, l, l_v, k, k_v, num_agebrackets=18, overwrite=True):
    file_name = f"{location}_new_case_per_day_{num_agebrackets}_{p}={p_v:.2f}_{l}={l_v:.2f}_{k}={k_v:.2f}.csv"
    file_path = os.path.join(output_source_dir, 'age-structured_output_source', p+"_"+str(p_v), location, file_name)
    e = os.path.exists(file_path)
    if e:
        print(f'{file_path} already exists.')
        if overwrite:
...
def dou_seaiq_with_age_specific_contact_martix(contact_matrix_dic, weights, ages, p, w, beta, q, l, k,
    ...
    initial_infected_number = min(5, ages[initial_infected_age])
    ...
    for t in range(timesteps):
        for i in range(num_agebrackets):
                ...
                for kk, setting in enumerate(settings):
                    ...
                    incidenceV_by_setting[kk][i][t] += (1 - p) * w * beta * weights[setting] * \
                                                       ...
                                                       states[indices['susceptible']][i][t] * (
                                                               ...
                                                               states[indices['asymptomaticN']][j][t] +
                                                               states[indices['asymptomaticV']][j][t]) / ages[j]
            # print("aaaaaaa", incidenceN_by_setting[:, i, t].sum()+incidenceV_by_setting[:, i, t].sum())
            if (states[indices['susceptible']][i][t] - incidenceN_by_setting[:, i, t].sum() - incidenceV_by_setting[:, i, t].sum() < 0):
                states[indices['susceptible']][i][t + 1] = 0
            else:
                states[indices['susceptible']][i][t + 1] = states[indices['susceptible']][i][t] - incidenceN_by_setting[:, i, t].sum() - incidenceV_by_setting[:, i, t].sum()
            # print(states[indices['susceptible']][i][t + 1])
            states[indices['exposedN']][i][t + 1] = states[indices['exposedN']][i][t] + incidenceN_by_setting[:, i, t].sum() - sigma * states[indices['exposedN']][i][t]
            states[indices['exposedV']][i][t + 1] = states[indices['exposedV']][i][t] + incidenceV_by_setting[:, i, t].sum() - sigma * states[indices['exposedV']][i][t]
            states[indices['infectedN']][i][t + 1] = states[indices['infectedN']][i][t] + q * sigma * states[indices['exposedN']][i][t] - gamma * states[indices['infectedN']][i][t]
            states[indices['infectedV']][i][t + 1] = states[indices['infectedV']][i][t] + l * q * sigma * states[indices['exposedV']][i][t] - gamma * states[indices['infectedV']][i][t]
            states[indices['asymptomaticN']][i][t + 1] = states[indices['asymptomaticN']][i][t] + (1 - q) * sigma * states[indices['exposedN']][i][t] - k * gamma * states[indices['asymptomaticN']][i][t]
            states[indices['asymptomaticV']][i][t + 1] = states[indices['asymptomaticV']][i][t] + (1 - q * l) * sigma * states[indices['exposedV']][i][t] - k * gamma * states[indices['asymptomaticV']][i][t]
            states[indices['quarantined']][i][t + 1] = states[indices['quarantined']][i][t] + gamma * (states[indices['infectedV']][i][t] + states[indices['infectedN']][i][t]) + k * gamma * (states[indices['asymptomaticN']][i][t] + states[indices['asymptomaticV']][i][t])
            ...
            cumulative_incidence_age[i][t] = (states_increase[indices['asymptomaticN']][i].sum() + states_increase[indices['asymptomaticV']][i].sum() + states_increase[indices['infectedN']][i].sum() + states_increase[indices['infectedV']][i].sum()) / ages[i] * 100
        cumulative_incidence[t] = states_increase[3:7].sum()/total_population * 100
    return states, incidenceN_by_setting, incidenceV_by_setting, states_increase, cumulative_incidence_age, cumulative_incidence, indices, setting_indices
...
def get_ages(location, num_agebrackets=18):
    """
    ...
    file_name = 'China_subnational_' + location + '_age_distribution_' + '%i' % num_agebrackets + '.csv'
...
# %% 获取接触矩阵字典
def get_contact_matrix_dic(location, num_agebrackets=18):
    """
...
# %% 读取接触矩阵
def read_contact_matrix(location, setting, num_agebrackets=18):
    """
...
# 保存每个年龄段的累计发病率
def write_cumulative_incidence_age(location, cumulative_incidence, p, p_v, l, l_v, k, k_v, num_agebrackets=18, overwrite=True):
...
# 不分年龄的累计发病率
def write_cumulative_incidence(location, cumulative_incidence, p, p_v, l, l_v, k, k_v, num_agebrackets=18, overwrite=True):
...
timesteps = 700  # how long to run the SEAIQ model
...
weights3 = {'household': 2.11, 'school': 5.41, 'work': 4.07, 'community': 1.79}
weights4 = {'household': 2.11, 'school': 1.41, 'work': 2.07, 'community': 1.79}
...
for location in location_dic:
    ages = get_ages(location, num_agebrackets)
    ...
    ccm = combine_synthetic_matrices(contact_matrix_dic, weights4, num_agebrackets)
    ...
    for i in range(11):
        ...
        for j in range(11):
            ...
            states, incidenceN, incidenceV, states_increase, cumulative_incidence_age, cumulative_incidence, indices, setting_indices = dou_seaiq_with_age_specific_contact_martix(
                contact_matrix_dic, weights4, ages, p_v, w_v, beta_v, q_v, l_v, k_v, susceptibility_factor_vector,
                sigma_inverse, gamma_inverse, initial_infected_age, percent_of_initial_infected_seeds,
                num_agebrackets, timesteps)
            # write_data(location, state, np.sum(states, axis=1).T, p, p_v, l, l_v, k, k_v, num_agebrackets=18)
            new_case_per_day = np.round(np.sum(states_increase, axis=1).T)
            write_per_day_new_cases(location, new_case_per_day, p, p_v, l, l_v, k, k_v, num_agebrackets)
            ...
            # write_cumulative_incidence(location, cumulative_incidence, p, p_v, l, l_v, k, k_v, num_agebrackets)
            # write_cumulative_incidence_age(location, cumulative_incidence_age.T,p,p_v,l,l_v,k,k_v,num_agebrackets)

# print("年龄分布：", ages)
# print("当前参数：", 'p =', p_v, ', w =', w_v, ', beta =', beta_v, ', q =', q_v, ', l =', l_v, ', sigma =', sigma_inverse, ', k =', k_v, ', gamma =', gamma_inverse)
...
# print('R0:', R0)
# print("感染者总数：", states_increase[3:7].sum())
# print("感染率：", states_increase[3:7].sum()/sum(ages.values()))
# print(states)
# # print(states[:,1,:])
# print(np.sum(states, axis=1).shape)
...
for i in range(11):
    ...
    for j in range(0, 11):
        ...
        result_file_path = write_data(country, location, state, np.round(all_states.T), p, p_v, l, l_v, k, k_v, num_agebrackets, overwrite=True)
        ...
        write_per_day_new_cases(country, location, new_case_per_day, p, p_v, l, l_v, k, k_v, num_agebrackets, overwrite=True)
