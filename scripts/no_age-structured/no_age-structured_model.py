...
def write_per_day_new_cases(new_case_per_day, p, p_v, l, l_v, k, k_v, overwrite=True):
    ...
    file_path = os.path.join(output_source_dir, 'no_age-structured_output_source', 'per_day', file_name)
    e = os.path.exists(file_path)
    if e:
        print(f'{file_path} already exists.')
        if overwrite:
...
def write_data_asy(state, value, p, p_value, l, l_value, k, overwrite=True):
    file_name = f"{state}_numbers_{p}={p_value:.1f}_{l}={l_value:.1f}_{k}.csv"
    file_path = os.path.join(output_source_dir, 'no_age-structured_output_source', 'mutil_params', file_name)
    e = os.path.exists(file_path)
    if e:
...
def write_cumulative_incidence(value, p, p_value, l, l_value, k, overwrite=True):
    ...
    return file_path
# %% 将得到的结果读入csv文件中
def write_cumulative_incidence_single(value, p, p_value, l, l_value, k, k_v, overwrite=True):
...
# 写入累计感染人数
def write_cumulative_infectious_number(value, p, p_value, l, l_value, k, k_v, overwrite=True):
    file_name = f"cumulative_infectious_number_{p}={p_value:.1f}_{l}_{l_value}_{k}_{k_v}.csv"
    file_path = os.path.join(output_source_dir, 'no_age-structured_output_source', 'cumulative_infectious_number', file_name)
    e = os.path.exists(file_path)
    if e:
        print(f'{file_path} already exists.')
        if overwrite:
...
def simulation_with_no_age_structured_model(ages, p, w, beta, q, l, sigma_inverse, k, gamma_inverse,
    ...
    cumulative_infectious_number = np.zeros((8, timesteps + 1))
    ...
    for t in range(timesteps):
        ...
        states_increase[indices['quarantined']][t + 1] = gamma * (
                states[indices['infectedV']][t] + states[indices['infectedN']][t]) + k * gamma * (
                                                                 ...
                                                                 states[indices['asymptomaticV']][t])
        # 累计感染人数
        cumulative_infectious_number[indices['asymptomaticN']][t+1] = states_increase[indices['asymptomaticN']].sum()
        cumulative_infectious_number[indices['asymptomaticV']][t+1] = states_increase[indices['asymptomaticV']].sum()
        cumulative_infectious_number[indices['infectedN']][t+1] = states_increase[indices['infectedN']].sum()
        cumulative_infectious_number[indices['infectedV']][t+1] = states_increase[indices['infectedV']].sum()

        # 累计发病率
        cumulative_incidence[t] = (states_increase[indices['asymptomaticN']].sum() + states_increase[
            indices['asymptomaticV']].sum() + states_increase[indices['infectedN']].sum() + states_increase[
                                       indices['infectedV']].sum()) / sum(ages.values()) * 100

    return states, incidenceN, incidenceV, indices, states_increase, cumulative_infectious_number, cumulative_incidence
...
def get_R0(beta, p, w, q, l, k, gamma_inverse):
    ...
    R0 = (beta / (k * gamma)) * (p * ((1 - q) + k * q) + (1 - p) * w * ((1 - q * l) + k * q * l))
...
print("年龄分布：", ages)
print("当前参数：", 'p =', p_v, ', w =', w_v, ', beta =', beta, ', q =', q_v, ', l =', l_v, ', sigma =', sigma_inverse, ', k =', k_v, ', gamma =', gamma_inverse)
...
# for j in range(0, 11):
#     l_v = j / 10.0
states, incidenceN, incidenceV, indices, state_increase, cumulative_infectious_number, cumulative_incidence = simulation_with_no_age_structured_model(
    ages, p_v, w_v, beta,
    q_v, l_v,
    sigma_inverse, k_v,
    gamma_inverse,
    ...
    timesteps)
print(cumulative_incidence.sum())
print(state_increase.shape)
print(state_increase[5])

# 将每次运行的结果存起来
# result_file_path = write_data(state, np.round(states.T), p, p_v, l, l_v, k, k_v, overwrite=True)
# write_param_value(p_v, w_v, beta, q_v, l_v, sigma_inverse, k_v, gamma_inverse, result_file_path)
# # 将每日增长的病例保存起来
new_case_per_day = state_increase.T
write_per_day_new_cases(new_case_per_day, p, p_v, l, l_v, k, k_v, overwrite=True)
# write_cumulative_infectious_number(cumulative_infectious_number.T, p, p_v, l, l_v, k, k_v, overwrite=True)

"""                   接种率对发病率的影响              """
# 控制其他参数不变的情况下，p 分别为 0.1 0.5 0.9
# write_cumulative_incidence_single(cumulative_incidence, p, p_v, l, l_v, k, k_v, overwrite=True)
...
#         quarantined = states[indices['quarantined']]
        ...
        # 将每次运行的结果存起来
        # result_file_path = write_data(state, states.T, p, p_v, l, l_v, k, k_v, overwrite=True)
        # write_param_value(p_v, w_v, beta, q_v, l_v, sigma_inverse, k_v, gamma_inverse, result_file_path)
        # 将每日增长的病例保存起来
